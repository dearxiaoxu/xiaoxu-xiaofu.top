...
纯原生 **HTML / CSS / JavaScript + 零依赖 Node.js** 打造，自带**管理后台**与**数据存储**：所有内容（文章、项目、相册、留言等）都保存在 `data/db.json` 里，通过浏览器后台可视化编辑，代码里不再有任何需要手动修改的内容数据。

## 快速开始

无需 `npm install`，只要本机装有 Node.js（≥14）：
...
├── data/db.json           # ★ 全部内容数据（唯一数据源，后台编辑的就是它）
...
- **数据与安全**：一键导出/导入 db.json 备份、修改管理员密码
...
- 全部内容在 `data/db.json`（写入采用原子替换，不会写坏文件）；
- 上传图片在 `data/uploads/`，建议定期备份整个 `data/` 目录；
- 后台「数据与安全」页支持一键下载 db.json 备份、导入恢复。
...
> 若仍想用纯静态托管（Cloudflare Pages 等），本版的管理后台与存储不可用——那是上一版的能力范围。需要的话可以额外做一个「静态导出」脚本把 db.json 渲染成纯 HTML。
